...
class undergoes(db.Model):
    ...
    pat_id = db.Column(db.Integer, db.ForeignKey('patient.pat_id'))
    proc_code = db.Column(db.Integer,db.ForeignKey('procedures.code'))
...
class Signup(UserMixin,db.Model):
    ...
    usertype = db.Column(db.Enum('patient', 'doctor', 'nurse'), nullable=False)

    def __repr__(self):
        return f"Signup ID: {self.user_id}, Email: {self.email}, Username: {self.username}, User Type: {self.usertype}"
...
def addpatient():
    if request.method == 'POST':
        ...
        db.session.commit()
    # Patient table has 5 entries
    # flash("Patient table has 3 entries","success")           
...
def adddoctor():
    if request.method == 'POST':
        ...
        db.session.commit()
    # flash("Doctor table has 5 entries","success")           
...
def addnurse():
    if request.method == 'POST':
        ...
        db.session.commit()
    # flash("Nurse table has 2 entries","success")           
...
def addappoitment():
    if request.method == 'POST':
        ...
        db.session.commit()
    # flash("Appointment table has 4 entries","success")     
    subject="HOSPITAL MANAGEMENT SYSTEM"
    mail.send_message(subject, sender=params['gmail-user'], recipients=[current_user.email],body=f"YOUR BOOKING IS CONFIRMED THANKS FOR CHOOSING US")      
...
def addroom():
    if request.method == 'POST':
        ...
        db.session.add(Room)
        db.session.commit()
    # flash("Room table has 5 entries","success")           
...
def addmedication():
    if request.method == 'POST':
        ...
        db.session.commit()
    # flash("Medication table has 4 entries","success")           
...
def adddepartment():
    if request.method == 'POST':
        ...
        db.session.commit()
    # flash("Depatment table has 3 entries","success")           
...
def addprocedure():
    if request.method == 'POST':
        ...
        Procedure = procedures(code=code, name=name, cost=cost)
        db.session.add(Procedure)
        db.session.commit()
    # flash("Procedure table has 6 entries","success")           
    allprocedure = procedures.query.all()
...
def addundergoes():
    if request.method == 'POST':
        ...
        db.session.commit()
    # flash("Room capacity is reached maximmum","danger")           
...
def addprescribes():
    if request.method == 'POST':
        ...
        db.session.commit()
    # flash("prescribes table has 5 entries","success")           
...
def updateprocedure(code):
    if request.method == 'POST':
        name = request.form['name']
        cost = request.form['cost']
        Procedure = procedures.query.filter_by(code=code).first()
        Procedure.name = name
        Procedure.cost = cost
        db.session.add(Procedure)
        db.session.commit()
    ...
    allprocedure = procedures.query.filter_by(code=code).first()
...
def signup():
    if request.method == "POST":
        ...
        with db.engine.begin() as conn:
            # result = conn.execute(text(f"INSERT INTO `signup` (`username`,`email`,`password`) VALUES ('{username}','{email}','{encpassword}');")) 
            result = conn.execute(text(f"INSERT INTO `signup` (`email`,`username`,`password`,`usertype`) VALUES ('{email}','{username}','{encpassword}','{usertype}');")) 
            conn.commit() 
...
def login():
    if request.method == "POST":
        email=request.form.get('email')
        password=request.form.get('password')
        usere=Signup.query.filter_by(email=email).first()

        if usere and check_password_hash(usere.password,password):
        # print(user.password)
